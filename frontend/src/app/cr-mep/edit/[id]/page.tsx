...
export default function EditCRMEPPage() {
  ...
  const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; title: string }>({
    isOpen: false,
    title: '',
  });

  // Charger les données du rapport au montage du composant
  useEffect(() => {
    const loadReport = async () => {
      const id = params.id as string;

      if (!id) {
        toast.error('ID du rapport manquant', {
          duration: 3000,
          position: 'bottom-right',
        });
        router.push('/cr-mep/create');
        return;
      }
      ...
      try {
        ...
        const reportDetails = await fileReportService.get(id);
        ...
        // Initialiser le localStorage avec les données chargées
        localStorage.setItem('crmep-report-data', JSON.stringify(reportDetails.data));

        // Stocker l'ID du rapport
        setCurrentReportId(id);

        // Stocker les métadonnées pour l'affichage
        setReportMetadata({
          package: reportDetails.metadata.package,
          sprint: reportDetails.metadata.sprint,
          version: reportDetails.metadata.version,
          updatedAt: reportDetails.metadata.updatedAt,
        });

        // Wait for component to mount before dispatching reload event
        setTimeout(() => {
        ...
        toast.error(
          `Erreur lors du chargement: ${error instanceof Error ? error.message : 'Unknown error'}`,
          {
            duration: 4000,
            position: 'bottom-right',
          }
  ...
  const handleSave = async () => {
    if (isSaving || !currentReportId) return;
    ...
    try {
      ...
      toast.loading('Mise à jour du rapport...', {
        duration: 2000,
        position: 'bottom-right',
      });
      ...
      setReportMetadata({
        package: savedReport.package,
        sprint: savedReport.sprint,
        version: savedReport.version,
        updatedAt: savedReport.updatedAt,
      });

      toast.success(`Rapport mis à jour avec succès (v${savedReport.version})`, {
        duration: 3000,
        position: 'bottom-right',
        style: {
          background: '#1C355E',
  ...
  const handleExport = async (format: 'pdf' | 'html') => {
    ...
    try {
      ...
      if (format === 'html') {
        ...
        a.href = url;
        a.download = `rapport_mep_${reportData.package_version}.html`;
  ...
  const handleDelete = () => {
    if (reportMetadata) {
      setDeleteModal({
        isOpen: true,
        title: `CR MEP ${reportMetadata.package} - Sprint ${reportMetadata.sprint}`,
      });
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await fileReportService.delete(currentReportId);
        ...
        <div className="relative z-10 flex items-center justify-center min-h-screen">
          <div className="text-center">
            <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-t-2"
                 style={{ borderColor: theme === 'dark' ? '#CC9F53' : '#1C355E' }}></div>
            <p className="mt-4 text-lg" style={{ color: theme === 'dark' ? '#CC9F53' : '#1C355E' }}>
        ...
        >
          ...
          >
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <ReportTopBar
                title={reportMetadata ? `Modifier CR MEP ${reportMetadata.package} - Sprint ${reportMetadata.sprint}` : 'Modifier CR MEP'}
                metadata={reportMetadata ? {
                  fileName: `rapport_mep_${reportMetadata.package}.html`,
                  version: reportMetadata.version,
                  lastModified: new Date(reportMetadata.updatedAt).toLocaleString('fr-FR')
                } : undefined}
